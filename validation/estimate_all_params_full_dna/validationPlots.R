...
test_vals2 <- function(prob, true=0, estimated=c()){
  lower <- hdr(estimated, prob=prob)$hdr[1]
  upper <- hdr(estimated, prob=prob)$hdr[2]
  test <- c(as.numeric(true >= lower & true <= upper))
  return(test)
}
...
wd<-"/Users/jugne/Documents/Source/beast2.7/sRanges-material/validation/estimate_all_params_full_dna/"
setwd(wd)
dir.create("figures")

# read in the true rates
true_rates_file <- "/Users/jugne/Documents/Source/beast2.7/sRanges-material/validation/estimate_all_params_full_dna/true_rates.csv"
...
for (i in 1:n){
  ...
  # print(HPDinterval(as.mcmc(t$samplingProportion), 0.05))
  # print(hdr(t$samplingProportion, 5)$hdr)
  # print(calc_hpd_interval(t$samplingProportion, 0.05))
  # print(samplingProportion$true[i])
