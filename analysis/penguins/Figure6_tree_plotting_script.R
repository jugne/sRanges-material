...
sa_label_size     <- 14    # text size
sa_nudge_x        <- -1.5  # horizontal nudge (negative = left)
...
irange_label_size <- 14    # text size
irange_nudge_x    <- 2.5     # horizontal nudge (positive = right)
irange_nudge_y    <- 2.7   # vertical nudge magnitude (sign chosen automatically)
...
tip_label_size    <- 14    # text size
tip_nudge_x       <- 1   # horizontal nudge
tip_nudge_y       <- -0.3   # vertical nudge
tip_force         <- 2     # repulsion between labels
...
p1 <- ggplot(tt, aes(x, y, color=range_mod)) + geom_tree(linewidth=2.5) + geom_rootedge(colour="azure4",rootedge = 3, linewidth=2.5) +
  ...
  geom_point2(aes(subset=(node %in% which(species %in% c("Marplesornis_novaezealandiae", "Madrynornis_mirandus")))),
              color="red",size=highlight_tri_size, shape=17)
...
ggsave(paste0(figure6_dir,"/penguin_map_plot_3.pdf"),p5, width=42, height=45, limitsize = FALSE)
