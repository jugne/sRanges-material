...
write.csv(rbind(sp_first, sp_both),paste0(figure_path,"penguin_range_dates.csv"))
...
log <- read.table(paste0(sRangesBothPath,"/tipAgeRelog/penguins_inf_morph_at_both.tipAge.log"), header = T, )
log1 <- read.table(paste0(sRangesPath,"/tipAgeRelog/penguins_inf_morph_at_start.tipAge.log"), header = T, )
name <- "combined"
...
ggsave(paste0(figure_path,"penguin_SRFBD_",name,"_IQR.pdf"), width=14, height=7)
