...
wd <-  "/Users/jugne/Documents/Source/beast2.7/sRanges-material/validation/compound_prior_simulations/"
setwd(wd)
dir.create("figures")

# read in the true rates
# true_rates_file <- "/Users/jugne/Documents/Source/beast2.7/sRanges-material/validation/estimate_all_params/true_rates.csv"
# true.rates <- read.table(true_rates_file, header=TRUE, sep=",")

lines<-readLines("/Users/jugne/Documents/Source/beast2.7/sRanges-material/validation/compound_prior_simulations/Parameters.txt")
params_all <- str_split(lines, "\n")
param_names<- c("origin", "rho", "div_rate", "sampling_prop", "turnover", "lambda", "mu", "psi")

true.rates<- as.data.frame(matrix(str_split_i(params_all, "=", 2),nrow = 100,ncol = 8, byrow=T))
colnames(true.rates) <- param_names
true.rates$psi <- str_split_i(true.rates$psi, ";", 1)
true.rates <-apply(true.rates, 2, function(x) as.numeric(x))
...
for (i in 1:n){
  ...
  used.rates = true.rates[i,];
  
  t <- read.table(log[[1]], header=TRUE, sep="\t")

  # take a 10% burnin
  t <- t[-seq(1,ceiling(length(t$diversificationRate)/30)), ]
...
p.diversificationRate <- ggplot(diversificationRate)+
  ...
  geom_point(aes(x=true, y=estimated), size=2) + xlim(c(x.min_diversificationRate, x.max_diversificationRate))+
  theme_minimal()
